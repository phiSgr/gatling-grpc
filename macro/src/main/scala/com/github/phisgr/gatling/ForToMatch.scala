...
object ForToMatch {
  def toMatch(c: blackbox.Context)(tree: c.Tree): c.Tree = {
    ...
    tree match {
      ...
      case Apply(TypeApply(Select(v, TermName("map")), _), List(Function(List(ValDef(_, name, _, _)), body))) =>
        matchValidation(v, name, q"io.gatling.commons.validation.Success { $body }")
      case tree =>
